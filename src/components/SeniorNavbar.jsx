...
import { Map, Package, Lock, ShieldCheck, MessageSquarePlus } from 'lucide-react';
...
export default function SeniorNavbar({ activeTab, setActiveTab, btnLevel = 3 }) {
  ...
  const tabs = [
    ...
    { id: 'feedback', label: '意見板', icon: MessageSquarePlus, color: 'text-indigo-400' }
  ];

  const getNavBtnConfig = (level) => {
    switch (level) {
      case 1:
        return { minWidth: '32px', minHeight: '22px', iconClass: 'w-3 h-3', textClass: 'text-[8px]', containerClass: 'py-0 px-0.5 rounded-md gap-0', showText: true };
      case 2:
        return { minWidth: '40px', minHeight: '28px', iconClass: 'w-3.5 h-3.5', textClass: 'text-[9px]', containerClass: 'py-0.5 px-1 rounded-lg gap-0', showText: true };
      case 3:
        return { minWidth: '48px', minHeight: '34px', iconClass: 'w-4 h-4', textClass: 'text-[10px] font-bold', containerClass: 'py-0.5 px-1 rounded-lg gap-0.5', showText: true };
      case 4:
        return { minWidth: '56px', minHeight: '40px', iconClass: 'w-4.5 h-4.5', textClass: 'text-[11px] font-bold', containerClass: 'py-1 px-1.5 rounded-xl gap-0.5', showText: true };
      case 5:
        return { minWidth: '64px', minHeight: '46px', iconClass: 'w-5 h-5', textClass: 'text-xs font-black', containerClass: 'py-1 px-2 rounded-xl gap-0.5', showText: true };
      default:
        return { minWidth: '48px', minHeight: '34px', iconClass: 'w-4 h-4', textClass: 'text-[10px] font-bold', containerClass: 'py-0.5 px-1 rounded-lg gap-0.5', showText: true };
    }
  };
