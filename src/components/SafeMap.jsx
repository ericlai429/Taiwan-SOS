...
export default function SafeMap({
    ...
    if (showDangerFlags) {
      decryptedFlags.forEach(flag => {
        ...
        const icon = L.divIcon({
          ...
          html: isUnlocked
            ? `<div class="relative flex flex-col items-center select-none" style="width:150px; height:60px;">
                <div class="bg-rose-600 text-white font-black px-2 py-1 rounded-xl shadow-2xl border-2 border-amber-300 text-xs flex items-center justify-center gap-1 whitespace-nowrap z-20">
                  <span>🚩</span>
                  <span class="truncate max-w-[95px]">${flag.title}</span>
            ...
            : `<div class="relative flex flex-col items-center select-none" style="width:140px; height:50px;">
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      {/* 🛡️ 智能網格分區 2：廣播發話列與時間軸 (右側保留 right-14 避讓右側側邊功能鈕，100% 零遮擋) */}
      <div className={`absolute z-[990] transition-all pointer-events-auto flex flex-col gap-1.5 ${
        isLandscape && isMobile
          ? 'left-16 right-14 bottom-2 max-w-lg'
          : 'left-[2vw] right-14 bottom-[calc(68px+env(safe-area-inset-bottom,0px))] sm:left-1/2 sm:-translate-x-1/2 sm:w-full sm:max-w-lg sm:right-auto sm:px-3'
      }`}>
        {showInvasion && (
          <InvasionPlaybackBar
      ...
      {/* 🛡️ 智能網格分區 3：右側側邊功能欄 (獨立動態定位於右側邊欄，水平與時間軸/發話欄完全隔離) */}
      <div className={`absolute z-[995] flex flex-col gap-1.5 transition-all ${
        isLandscape && isMobile
          ? 'left-16 bottom-3'
          : 'bottom-[calc(68px+env(safe-area-inset-bottom,0px))] right-[2vw] sm:bottom-3 sm:right-3'
      }`}>
        {/* 1. 我的位置 */}
